...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    public class SysOrgService : ISysOrgService, IDynamicApiController, ITransient
    {
        ...
        public async Task<dynamic> QueryOrgPageList([FromQuery] OrgPageInput input)
        {
            ...
            var dataScopeList =  await GetUserDataScopeIdList();
        ...
        public async Task AddOrg(OrgAddInput input)
        {
            ...
            {
                TenantId = newOrg.Entity.TenantId,
            ...
            {
                TenantId    = newOrg.Entity.TenantId,
                Account     =  input.Admin_User,
                Password    = "e10adc3949ba59abbe56e057f20f883e",
                Name        = input.Admin_User,
                NickName    = input.Admin_User,
                Email       = "",
                Phone       =  newOrg.Entity.Tel,
        ...
        {
            var entity = await _sysOrgRep.DetachedEntities.FirstOrDefaultAsync(u => u.Id == long.Parse(input.Id));
            return entity;
        }
        ...
        public async Task<List<OrgTreeNode>> GetOrgs()
        {
            var dataScopeList = new List<long>();
            if (!_userManager.SuperAdmin)
            {
        ...
        public async Task<List<OrgTreeNode>> GetOrgs2()
        {
            var dataScopeList = new List<long>();
            if (!_userManager.SuperAdmin)
            {
                ...
                if (dataScopes.Count < 1)
                    return null;
                dataScopeList = dataScopes;//GetDataScopeList(dataScopes);
            }
            var orgs = await _sysOrgRep.DetachedEntities.Where(dataScopeList.Count > 0, u => dataScopeList.Contains(u.Id))
                                                        .Where(u => u.Status == CommonStatus.ENABLE).OrderBy(u => u.Sort)
                                                        .Select(u => new OrgTreeNode
